...
在 Git 中，提交记录（Commit）就是时间点；分支（Branch）就是时间线。Git 有很多实践流派，偶魔采用的是 [GitHub Flow](https://docs.github.com/zh/get-started/using-github/github-flow) 简化版，基于人分支，永不删除。为了方便初学者理解，我用直白的语言表述：
...
本地改动，通过双审核机制，才更新到主分支上。这样，即解放了本地创作限制，又规范了线上协作流程。有效避免了同步网盘方案中，高频改动导致的文件冲突问题，以及 AI 操作导致的潜在安全风险。
...
```mermaid
flowchart TD
  changes(本地改动 Changes)
  staged(暂存改动 Staged Changes)
  dev(本地个人分支 dev)
  
  changes -- git add --> staged
  staged -- git commit --> dev
  dev -- 创作 --> changes
```

这部分是最普通不过的 Git 单人使用方式：通过「Add - Commit - 创作」的循环往复，项目不断发展，留下一条线性的提交历史，记录了每次改动的内容。

你可以不和团队发生任何联系，也能让 Git 发挥功能，帮你储存关键时间节点，用于查看、回溯等操作。我们不是为了协作而使用 Git，我们是为了让创作过程「有迹可循」，培养大家的好习惯。就像那些能记录绘画过程的软件一样，除了创作结果外，不觉得创作记录本身也很酷么？
...
flowchart TD
  dev-1(本地个人分支 dev)
  origin-dev(远程个人分支 origin/dev)
  origin-main(远程主分支 origin/main)
	
  dev-1 -- git push --> origin-dev
  origin-dev -- PR --> origin-main
  origin-main -- git pull origin main --> dev-1
```

这部分才是团队协作的体现，个人分支通过 PR 传给主分支，同时更新来自主分支的改动，构成了「个人」和「团队」的协同。PR 确保了个人内容被单向的、有监督地并入，保障了安全性、稳定性。

只要记住这个 ♾️ 双循环，你就完全理解了这套工作流程。
